...
Index Index::build(const fs::path&root,IndexOptions opt){
    ...
    std::vector<fs::path> ps;auto dop=fs::directory_options::skip_permission_denied|(opt.follow_symlinks?fs::directory_options::follow_directory_symlink:fs::directory_options::none);for(auto const&e:fs::recursive_directory_iterator(I->root,dop))if(e.is_regular_file())ps.push_back(e.path());std::sort(ps.begin(),ps.end());
    for(auto const&p:ps){std::ifstream f(p,std::ios::binary);if(!f)continue;std::string s((std::istreambuf_iterator<char>(f)),{});FileInfo fi;fi.path=p.lexically_relative(I->root).generic_string();fi.size=s.size();fi.mtime_ns=mtime_ns(p);fi.binary=looks_binary(s);I->corp_bytes+=s.size();for(unsigned char c:s)++I->byte_freq[c];if(s.size()>=4)for(size_t q=0;q+4<=s.size();++q){auto h=detail::hash4((const unsigned char*)s.data()+q)&65535u;if(I->qgram_freq[h]!=UINT32_MAX)++I->qgram_freq[h];}I->infos.push_back(fi);I->loaded.push_back({fi,std::move(s)});}
...
bool Index::fresh()const{
    ...
    auto dop=fs::directory_options::skip_permission_denied|(impl_->opt.follow_symlinks?fs::directory_options::follow_directory_symlink:fs::directory_options::none);for(auto const&e:fs::recursive_directory_iterator(impl_->root,dop,ec)){
        if(ec){ec.clear();continue;} if(e.is_regular_file())current.push_back(e.path().lexically_relative(impl_->root).generic_string());
    }
    std::sort(current.begin(),current.end());
    if(current.size()!=impl_->infos.size())return false;
    for(size_t k=0;k<current.size();++k){
        if(current[k]!=impl_->infos[k].path)return false;
        auto p=impl_->root/impl_->infos[k].path;std::error_code x;
...
void Index::save(const fs::path&file)const{
    ...
    o.write("PERGREP\0",8);put(o,uint32_t(4));puts(o,impl_->root.string());
...
Index Index::load(const fs::path&file){
    ...
    auto I=std::make_shared<Impl>();I->root=gets(i);
    ...
    for(uint64_t k=0;k<nf;++k){FileInfo f;f.path=gets(i);f.size=get<uint64_t>(i);f.mtime_ns=get<int64_t>(i);f.binary=get<uint8_t>(i)!=0;I->infos.push_back(f);std::ifstream src(I->root/f.path,std::ios::binary);if(!src)throw std::runtime_error("indexed source disappeared: "+f.path);std::string data((std::istreambuf_iterator<char>(src)),{});I->loaded.push_back({f,std::move(data)});}
