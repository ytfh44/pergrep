...
[[noreturn]] void vm_throw_limit(const VerifierContext& c,const char* reason,const char* message){if(c.tm)c.tm->limit_reason=reason;throw std::runtime_error(message);}
std::vector<State> eval(const std::shared_ptr<RegexNode>&n,const VerifierContext& c,const PatternOptions&o,int depth,const State&s) {
    if(depth>10000)vm_throw_limit(c,"recursion-depth","pergrep regex: recursion depth exceeded");if(c.tm&&depth>=0&&static_cast<std::uint64_t>(depth)>c.tm->max_depth)c.tm->max_depth=static_cast<std::uint64_t>(depth);
    ...
    switch(n->kind){
        ...
        case K::Concat:{std::vector<State>cur{s};for(auto&ch:n->children){std::vector<State>next;for(auto&st:cur){auto v=eval(ch,c,o,depth+1,st);next.insert(next.end(),std::make_move_iterator(v.begin()),std::make_move_iterator(v.end()));if(next.size()>50000)vm_throw_limit(c,"vm-state-limit","pergrep regex: VM state limit exceeded");}cur.swap(next);if(cur.empty())break;}out=std::move(cur);break;}
        case K::Alt:{for(auto&ch:n->children){auto v=eval(ch,c,o,depth+1,s);out.insert(out.end(),std::make_move_iterator(v.begin()),std::make_move_iterator(v.end()));if(out.size()>50000)vm_throw_limit(c,"vm-state-limit","pergrep regex: VM state limit exceeded");}break;}
        ...
        case K::LookBehind:{VerifierContext look=c;if(c.left_context_available)look.record_begin=c.source_begin;if(c.right_context_available)look.record_end=c.source_end;std::size_t floor=n->negative||!c.left_context_available?c.record_begin:c.source_begin;std::size_t lo=s.pos>8192?std::max(floor,s.pos-8192):floor;{if(c.tm){c.tm->lookbehind_evals++;if(s.pos>=lo){const std::uint64_t w=s.pos-lo;if(w>c.tm->max_lookbehind_window)c.tm->max_lookbehind_window=w;}if(s.pos>8192)c.tm->lookbehind_capped++;}}if(n->negative){bool ok=false;for(std::size_t p=lo;p<=s.pos;){State q=s;q.pos=p;auto v=eval(n->children[0],look,o,depth+1,q);for(auto&z:v)if(z.pos==s.pos){ok=true;break;}if(ok||p==s.pos)break;auto rr=context_rune_at(c,p);p=rr.ok?rr.next:p+1;}if(!ok)out.push_back(s);}else{for(std::size_t p=lo;p<=s.pos;){State q=s;q.pos=p;auto v=eval(n->children[0],look,o,depth+1,q);for(auto&z:v)if(z.pos==s.pos){State r=z;r.pos=s.pos;out.push_back(std::move(r));}if(p==s.pos)break;auto rr=context_rune_at(c,p);p=rr.ok?rr.next:p+1;}}break;}
        case K::Repeat:{std::vector<State>levels{s};std::vector<std::vector<State>>all{levels};std::size_t total=levels.size();std::size_t hard=c.record_end>=s.pos?c.record_end-s.pos+1:0;std::size_t limit=n->max==SIZE_MAX?std::min<std::size_t>(hard,10000):std::min<std::size_t>(n->max,10000);{if(c.tm){const std::uint64_t natural=n->max==SIZE_MAX?hard:n->max;if(limit<natural)c.tm->repeat_capped++;}}for(std::size_t k=1;k<=limit;++k){if(c.tm)c.tm->repeat_iterations++;std::vector<State>next;for(auto&st:levels){auto v=eval(n->children[0],c,o,depth+1,st);for(auto&z:v)if(z.pos!=st.pos)next.push_back(std::move(z));}if(next.empty())break;if(next.size()>50000)vm_throw_limit(c,"vm-state-limit","pergrep regex: VM state limit exceeded");total+=next.size();if(total>50000)vm_throw_limit(c,"vm-state-limit","pergrep regex: VM state limit exceeded");all.push_back(next);levels=std::move(next);}if(n->greedy){for(std::size_t k=all.size();k-->n->min;)out.insert(out.end(),all[k].begin(),all[k].end());}else{for(std::size_t k=n->min;k<all.size();++k)out.insert(out.end(),all[k].begin(),all[k].end());}if(out.size()>50000)vm_throw_limit(c,"vm-state-limit","pergrep regex: VM state limit exceeded");break;}
    }
    if(c.tm)c.tm->state_expansions+=static_cast<std::uint64_t>(out.size());
    return out;
}
