...
### M2.4 bounded-region boundary contract

`RegexBoundedRegion` is the regular finite-width path selected from the same chunk candidates as `RegexChunk`. It is eligible with sensitive `word`, `line`, `multiline`, `dotall`, `unicode`, and `crlf` modes when metadata proves finite, non-nullable execution and mandatory literals. Records are split exactly as in the full verifier; multiline uses the complete file. NUL payloads and custom separators are preserved.

The execution region clips only rune consumption. Anchors use source/record bounds and separator policy, and word assertions decode adjacent source runes even across a region edge. Thus region endpoints cannot create a match or suppress one as an implicit file/record boundary. Extended look-around/backreference patterns, unbounded or ambiguous widths, and insensitive patterns retain the existing full verifier fallback.
