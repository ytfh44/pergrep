...
The standalone region-verifier test executable runs 96 seeded cases across anchors, separators, UTF-8 bytes, captures, bounded repeats, branch alternatives, lookaround, overlap, NUL records, and chunk edges. It compares indexed and manually bounded execution against the exact full-reference verifier; failures print seed, escaped corpus, options, plan hash, operator, fallback reason, and a deterministic minimized corpus before returning nonzero, so CI blocks the bounded operator on any counterexample.
