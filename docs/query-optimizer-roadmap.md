...
A bounded-region plan is admitted only for finite, known byte/rune widths with no VM resource-limit approximation. The current Searcher supplies source and record boundary context through VerifierContext, so finite boundary-sensitive plans remain eligible; a planner without that context must use the exact verifier and report missing-boundary-context. Unbounded repeats (including .*), backreferences, lookaround, folded-width uncertainty, and resource-capped repeats are rejected explicitly. Rejected plans clear their candidate set and invoke the exact per-file regex verifier over every indexed chunk; they never become an empty candidate rejection. SearchStats::verifier_fallback, physical_operator, and qgram_fallback_reason identify the fallback and reason (unbounded-repeat, backreference, lookaround, unknown-unicode-width, repeat-resource-limit, or missing-boundary-context).
### M2.8 — Region-Verifier Differential and Property Tests
The standalone region-verifier test executable runs 140 seeded cases across anchors, separators, UTF-8 bytes, captures, bounded repeats, branch alternatives, lookaround, overlap, NUL records, and chunk edges. It compares indexed execution and the production bounded-region verifier against an independent byte-level oracle; failures print seed, escaped corpus, options, plan hash, operator, fallback reason, actual/expected matches, and a deterministic minimized corpus before returning nonzero, so CI blocks the bounded operator on any counterexample.
