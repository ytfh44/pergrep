...
* `candidate_begin/end` is the planned half-open range of start offsets to try. The end may be `record_end + 1` to include an empty match at the record end. This range limits start attempts only; it does not narrow bytes visible to an attempt. M2.3 additionally passes absolute bounded-region coordinates for guarded finite-width regex verification.
