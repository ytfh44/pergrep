...
> **Scope note:** the on-disk index persists q-gram/positional filter structures and file metadata. New saves use portable index format v7: `PERGREP\0` magic, schema 2, fixed-width fields, and explicit little-endian scalar/vector encoding. With `IndexOptions::persist_corpus=false`, `Index::load()` re-reads source files for search; with it enabled, v7 advertises an optional persisted-corpus section. `Index::load()` still reads legacy v5/v6 snapshots, and migration is load-old/save-new rather than in-place rewriting. `Index::fresh()` re-traverses the tree and compares path/size/mtime per file. The in-tree benchmark consumes the versioned deterministic workload matrix; it is not a rigorous cross-tool benchmark.
